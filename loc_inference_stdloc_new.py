...
def localize_set(
    ...
    for index, _ in enumerate(views):
        ...
        if use_detector:
            ...
            dense_desc_h = F.interpolate(
                dense_desc.unsqueeze(0),
                size=(gt_im.shape[2], gt_im.shape[3]),
                mode="bilinear",
                align_corners=False,
            ).squeeze(0)
            dense_desc_h = F.normalize(dense_desc_h, dim=0, p=2)
            # Generate heatmap
            heatmap = detector(dense_desc_h)
            # Post-process heatmap
            # 1. Zero out boundary values
            radius = 4
            boundary = 5
            heatmap[0, :boundary, :] = heatmap[0, -boundary:, :] = heatmap[0, :, :boundary] = heatmap[0, :, -boundary:] = 0
            # 2. Non-Maximal Suppression
            kp_scores = simple_nms(heatmap, nms_radius=radius).flatten()
            _, kp_ids = torch.topk(kp_scores, 512)
            position_mask = kp_scores > 0
            kp_ids = kp_ids[position_mask[kp_ids]]
            kp_mask = torch.zeros_like(kp_scores, dtype=torch.bool)
            kp_mask[kp_ids] = True
            # 3. Get keypoints
            kp_mask = kp_mask.reshape(gt_im.shape[2], gt_im.shape[3])
            gt_keypoints = torch.argwhere(kp_mask)
            gt_feature = dense_desc_h[:, gt_keypoints[:, 0], gt_keypoints[:, 1]]
            gt_feature = torch.transpose(gt_feature, 0, 1)
            gt_feature = mlp.decode(mlp(gt_feature))
            gt_keypoints = torch.cat(
                [gt_keypoints[:, 1].unsqueeze(1), gt_keypoints[:, 0].unsqueeze(1)], dim=1
            ) + 0.5
        ...
        # print(f"\rRotErr: {rotError} deg, TransErr: {transError} cm", end="")
    ...
    print()
    log_errors(error_foler, name, prior_rErr, prior_tErr, "prior")
