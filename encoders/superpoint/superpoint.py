...
def sample_descriptors_fix_sampling(keypoints, descriptors, s: int = 8):
    ...
    descriptors = torch.nn.functional.grid_sample(descriptors, keypoints.view(b, 1, -1, 2), mode="bilinear", align_corners=False)
    descriptors = torch.nn.functional.normalize(descriptors.reshape(b, c, -1), p=2, dim=1)
    return descriptors
        ...
        if self.conf.sparse_outputs:
            ...
            if max_kps > 0:
                if self.conf.randomize_keypoints_training and self.training:
                    ...
                    keypoints, scores = list(zip(*[sample_k_keypoints(k, s, max_kps) for k, s in zip(keypoints, scores)]))
                else:
            ...
            if self.conf.force_num_keypoints:
                keypoints = pad_and_stack(keypoints, max_kps, -2, mode="random_c",
                                            bounds=(0, data.get("image_size", torch.tensor(image.shape[-2:])).min().item(),),)
            ...
            if (len(keypoints) == 1) or self.conf.force_num_keypoints:
                ...
                if self.conf.legacy_sampling:
                    ...
                    desc = [sample_descriptors_fix_sampling(k[None], d[None], 8)[0] 
                            for k, d in zip(keypoints, dense_desc)]
