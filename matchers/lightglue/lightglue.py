...
from .losses import NLLLoss_lg
from .metrics import matcher_metrics
from .utils import get_homography_data, count_trainable_params
...
DATA_PATH = "/home/koki/gluetrain/data"
...
class LightGlue(nn.Module):
    ...
    def __init__(self, conf) -> None:
        ...
        self.loss_fn = NLLLoss_lg(conf.loss)
        state_dict = None
        ...
        if conf.weights is not None:
            ...
            if Path(conf.weights).exists():
                ...
                state_dict = torch.load(str(DATA_PATH / conf.weights), 
                                        map_location="cpu")
            else:
    ...
    def loss(self, pred, data):
        def loss_params(pred, i):
            la, _ = self.log_assignment[i](pred["ref_descriptors0"][:, i], 
        ...
        for i in range(N - 1):
            log_assignment_i = loss_params(pred, i)
            nll, _, _ = self.loss_fn(log_assignment_i, data, weights=gt_weights)
            ...
            losses["confidence"] += self.token_confidence[i].loss(pred["ref_descriptors0"][:, i], pred["ref_descriptors1"][:, i], 
                                                                  log_assignment_i["log_assignment"], pred["log_assignment"],) / (N - 1)
            del log_assignment_i
...
def test2():
    model = __main_model__({}).eval().to(device)
    data = get_homography_data(batch=4)
    pred = model(data)
    print(pred.keys())
...
# python -m matchers.lightglue.lightglue
