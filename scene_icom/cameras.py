...
def compute_camera_distance(C1_inv, C2_inv, alpha=1.0):
    """
    Computes the translation and rotation distances between two cameras.

    Parameters:
        C1, C2: numpy arrays of shape (4, 4)
            World-to-camera transformation matrices for camera 1 and camera 2.
        alpha: float
            Weighting factor to balance translation and rotation distances.

    Returns:
        d_translation: float
            Euclidean distance between the camera centers.
        theta: float
            Rotation angle difference in radians.
        d_pose: float
            Combined pose distance.
    """

    # Extract translation vectors (first three elements of the last column)
    T1 = C1_inv[:3, 3]
    T2 = C2_inv[:3, 3]

    # Compute translation distance
    d_translation = np.linalg.norm(T1 - T2)

    # Extract rotation matrices (top-left 3x3 submatrices)
    R1 = C1_inv[:3, :3]
    R2 = C2_inv[:3, :3]
    ...
    trace_R_rel = np.clip(np.trace(R_rel), -1.0, 3.0)

    # Compute rotation angle difference
    theta = np.arccos((trace_R_rel - 1) / 2)

    # Combine translation and rotation distances
    d_pose = d_translation + alpha * theta

    return d_translation, theta, d_pose
