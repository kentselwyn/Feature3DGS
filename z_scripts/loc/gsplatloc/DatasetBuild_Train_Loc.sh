...
if (( build_data )); then
    ...
    python dataset_build.py --source_path "$s_path_train" --mlp_method "$mlp_method" --mlp_dim $mlp_dim  --images rgb \
                            --output_images "$data_ImgName" \
                            --resize_num $resize_num --th $th --max_num_keypoints $max_num_keypoints --feat_name $feat_name
    python dataset_build.py --source_path "$s_path_test" --mlp_method "$mlp_method" --mlp_dim $mlp_dim --images rgb \
                            --output_images "$data_ImgName" \
                            --resize_num $resize_num --th $th --max_num_keypoints $max_num_keypoints --feat_name $feat_name
fi
