...
import os
import warnings
import utils.dataset_util as dutil
import numpy as np
import torch
...
src_folder = '/home/koki/code/cc/feature_3dgs_2/data/vis_loc/gsplatloc/7_scenes/7scenes_source'
focal_length = 525.0
...
# python -m utils.setup_7scenes --poses pgt
if __name__ == '__main__':
    ...
    def process_scene(ds):
        ...
        def link_frames(split_file, variant):
            ...
            if opt.poses == 'pgt':
                pgt_file = os.path.join('/home/koki/code/cc/feature_3dgs_2', external_pgt_folder, '7scenes', f'{ds}_{variant}.txt')
            ...
            for seq in split:
                ...
                for img in images:
                    os.system(
                        'ln -s ' + '/home/koki/code/cc/feature_3dgs_2/data/vis_loc/gsplatloc/7_scenes/7scenes_source' + '/' + ds + '/' + seq + '/' + img + ' ' + target_folder + variant + '/rgb/' + seq + '-' + img)
    ...
    Parallel(n_jobs=7, verbose=0)(map(delayed(process_scene), ['fire', 'heads', 'office', 'pumpkin', 
                                                               'redkitchen', 'stairs', 'chess']))
    # process_scene('chess')
