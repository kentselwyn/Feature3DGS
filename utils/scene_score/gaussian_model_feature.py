...
class GaussianModel:
    ...
    def load_ply_feature_grad(self, path, semantic_feature_size=256):
        plydata = PlyData.read(path)
        ...
        self._score_feature = torch.tensor(score_feature, dtype=torch.float, device="cuda").transpose(1, 2).contiguous()
        self.active_sh_degree = self.max_sh_degree
