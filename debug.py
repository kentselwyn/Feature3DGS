...
def localize_set(model_path, name, views, gaussians, pipe_param, background, 
    ...
    with torch.no_grad():
        for index, (image_B1HW, _, gt_pose_B44, _, intrinsics_B33, _, _, filenames) in enumerate(ace_test_loader):
            ...
            stdloc = STDLoc(model_path, gaussians)
            ...
            for _, (gt_pose_44, intrinsics_33, frame_path) in \
                            enumerate(zip(gt_pose_B44, intrinsics_B33, filenames)):
                ...
                pose_w2c, _ = stdloc.loc_sparse(gt_im, focal_length, focal_length)
                # Convert World-to-Camera to Camera-to-World
                out_pose = np.linalg.inv(pose_w2c)
                ...
                rotError, transError = loc_utils.calculate_pose_errors_ace(gt_pose_44, out_pose)
                print("STDLoc Time: ", time.time()-st0)
