            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    if render_func:
                        ...
                        render_pkg = render_gsplat(viewpoint, scene.gaussians, background, rgb_only=False, features_only=getattr(model_param, 'features_only', False))
                    ...
                    # Compute losses
                    if getattr(model_param, 'features_only', False):
                ...
                print("\n[ITER {}] Evaluating {}: L1 {:.6f} PSNR {:.2f} Feature L1 {:.6f} Score L1 {:.6f}{}".format(
                    ...
                    " (RGB metrics disabled in features-only mode)" if getattr(model_param, 'features_only', False) else ""
                ))
...
def training(model_param, opt_param, pipe_param, testing_iterations, saving_iterations, 
    ...
    if getattr(model_param, 'features_only', False):
        print("*** FEATURES ONLY MODE: Training without RGB supervision ***")
    else:
        print("*** STANDARD MODE: Training with full RGB + feature supervision ***")
    ...
    for iteration in range(first_iter, opt_param.iterations + 1):
        ...
        # Render the scene
        render_pkg = render_gsplat(viewpoint_cam, gaussians, background, rgb_only=False, features_only=getattr(model_param, 'features_only', False))
        ...
        # Compute RGB loss (skip if features_only is True)
        if getattr(model_param, 'features_only', False):
        ...
        # Total loss
        if getattr(model_param, 'features_only', False):
            # Features-only mode: exclude RGB and SSIM losses
            loss = (1.0 * Ll1_feature + 
                    getattr(model_param, 'score_scale', 1.0) * Ll1_score)
        else:
            # Standard mode: include all losses
...
if __name__ == "__main__":
    ...
    parser.add_argument("--features_only", action="store_true", default=False,
                       help="Train only with feature and score supervision, skip RGB supervision")
    ...
    # Start training
    model_params_extracted = model_params.extract(args)
    model_params_extracted.features_only = args.features_only
    training(model_params_extracted, opt_params.extract(args), pipe_params.extract(args),
