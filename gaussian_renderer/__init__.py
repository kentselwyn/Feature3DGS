...
def get_render_visible_mask(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    means3D.retain_grad()
    rendered_image.sum().backward(retain_graph=True)
    
    visible_mask = means3D.grad.norm(dim=-1) > 0
    means3D.grad.zero_()
    
    means3D.detach()
    rendered_image.detach()
    screenspace_points.detach()
